    ...
    public static void t1() {
        ...
        class DefaultComparator<T extends Comparable> implements Comparator<T> {
            ...
            public int compare(T o1, T o2) {
                return -o1.compareTo(o2); // returns the negative
            }
        }
        ...
        while(list.isEmpty() == false) {
            pq.add(list.remove(0)); //keeps removing elements from the head of the list and adds them to the queue as long as list is not empty
        }

        while(pq.isEmpty() == false) {
            list.add(pq.removeMinimum()); //dequeue priority queue => keep removing the minimum element and add it to the list which was emptied earlier
        }                                 // elements should be inserted in descending order
        for(Integer i : list) {
